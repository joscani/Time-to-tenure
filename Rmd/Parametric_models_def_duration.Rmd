---
title: 'Survival modelling.  time to tenure : (using Def Duration)'
...
nombre | descripcion
------ | -----------
TimePermCG7 | Tiempo permanencia
PermanentCG7 | variable de status: 1 tiene la plaza
mobilityPDurG2 | Movilidad
Q6 | dicipline
Q23 | Sex
Q24 | Age
Q25 | Single
...
datos <- datos[,c("ability","Countryb","Q6","Q23","Q24","Q25","Q27",
						...
						"multidisc", "TimePermCG7","PermanentCG7",
						"mobilityPDurG2")]
```
...
```{r}
datos$time <- datos$TimePermCG7

datos$status <- datos$PermanentCG7 
...
sumamos 0.001 a time (es por cuestión de cálculos)
...
S1[1:20]
```
Si lo vemos en los datos, vemos que el caso 3 no ha obtenido la plaza (status=0)
...
with(datos, tapply(time,mobilityPDurG2, function(x)c(media=mean(x),mediana=median(x))))

# solo lso que tienen status=1

with(datos[datos$status==1,], tapply(time,mobilityPDurG2, function(x)c(media=mean(x),mediana=median(x))))
...
```{r, fig.width=5}
mod1 <- npsurv(S1 ~ mobilityPDurG2 , datos)
survplot(mod1, ylab=expression(hat(S)(t)))
```


Para cada time y en cada categoría de mobilityPDurG2, se ha calculada el valor de survival y el intervalo de confianza. No se aprecian diferencias entre movilidad 0 y movilidad 1
...
survdiff(S1 ~ mobilityPDurG2 , datos)
...
Bajo la hipótesis de errores exponenciales, vemos que la disciplina no influye en la supervivencia (**_con las otras definiciones si pasaba_**) 
...
```{r}
mod8 <- survfit(S1 ~ Q6 + mobilityPDurG2, data = datos)
...
mod9 <- survreg(S1 ~ Q6 * mobilityPDurG2 * Countryb  , datos)
mod9.step <- step(mod9)
```

El procedimiento por pasos nos dice que la interacción significativa es la de movilidad y país. Esto va en línea con lo que pensábamos, la movilidad influye sobre los tiempos hasta conseguir la plaza de manera diferente en cada país..( **_pasa igual que con las anteriores definiciones_**) 
...
Las categorías de referencia en cada variable son Social Sciences para Q6, 0 para mobilityPDurG2 y United Kingdom para Countryb.. Lo podemos cambiar según que nos interese tener en cada categoría
...
res.km <- 
## Define a function to plot survreg prediction by 
...
survreg.curves <- function(model, col = "black",
    ...
    x = predict(model, newdata=toPredict,
                type="quantile",
                p = seq.quantiles)
    rownames(x) <- paste0("mobility = ",toPredict$mobilityPDurG2, " ,Discipline = ", toPredict$Q6, ", Country = ", toPredict$Countryb )  
    y = 1-seq.quantiles
...
dat1 <-  data.frame(Q6 = "Social Sciences", mobilityPDurG2 = c(0,1),
                         Countryb = "spain" )
...
# 
dat2 <- data.frame(Q6 = "Social Sciences", mobilityPDurG2 = c(0,1),
...
plot(survfit(S2 ~ mobilityPDurG2, subdatos))
