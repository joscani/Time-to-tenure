---
title: "Cox Models  time to tenure : (using Def Duration)"
...
Vamos a centrarnos en las variables de disciplina (Q6), movilidad (mobilityPDurG2) y país (Countryb)
...
```{r, warning=FALSE}
# me quedo con datos 
filtro <- !is.na(datos$Q6) & !is.na(datos$mobilityPDurG2)
table(filtro)
S1.nuevo <-  with(datos[filtro,], Surv(time,status))
mod.cph <- coxph(S1.nuevo ~ Q6 * mobilityPDurG2 * Countryb, 
...
mod.cph.step <- coxph(S1.nuevo ~ Q6 + mobilityPDurG2 * Countryb, 
    data = datos[filtro, ], method="breslow")
...
res.km <- npsurv( Surv(time,status) ~ mobilityPDurG2 , data=datos[filtro & datos$Countryb=="united kingdom" & datos$Q6=="Social Sciences",])
survplot(fit  = res.km,
...
lines(survfit(mod.cph.step,
              newdata = data.frame(mobilityPDurG2 = 0:1,
...
```{r, echo=FALSE}
res.km <- npsurv( Surv(time,status) ~ mobilityPDurG2 , data=datos[filtro & datos$Countryb=="spain" & datos$Q6=="Social Sciences",])
survplot(fit  = res.km,
         conf = c("none","bands","bars")[1],
         xlab = "", ylab = "Survival",
#          label.curves = TRUE,                     # label curves directly
...
```{r}
mod.cph.strat <- coxph(S1.nuevo ~ Q6 + mobilityPDurG2 + strata(Countryb), 
    data = datos[filtro, ], method = "breslow")
test.asun <- cox.zph(mod.cph.strat, transform="rank")
round(test.asun$table,3)
round(p.adjust(test.asun$table[,3], method = "bonferroni"),3)

mod.cph.strat2 <- coxph(S1.nuevo ~ Q6 + mobilityPDurG2 * strata(Countryb), 
    data = datos[filtro, ], method = "breslow")
...
```{r}
mod.cph.strat3 <- coxph(S1.nuevo ~  mobilityPDurG2 * strata(Countryb)+  strata(Q6), 
    data = datos[filtro, ], method = "breslow")
...
mod.cph.strat31 <- coxph(S1.nuevo ~  mobilityPDurG2 + strata(Countryb)+ strata(Q6), 
    data = datos[filtro, ], method = "breslow")
...
lapply(1:10, function(x){
                        survplot(
                            npsurv(Surv(time,status)~ mobilityPDurG2 ,
...
```{r, echo=FALSE}
res.km <- npsurv( Surv(time,status) ~ mobilityPDurG2 , data=datos[ datos$Countryb=="spain" & datos$Q6=="Social Sciences",])
...
```{r, echo=FALSE}
res.km <- npsurv( Surv(time,status) ~ mobilityPDurG2 , data=datos[ datos$Countryb=="united kingdom" & datos$Q6=="Social Sciences",])
